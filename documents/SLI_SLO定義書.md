...
- **トラフィック特性**: 通常時0.08 req/s、ピーク時0.8 req/s（配送トラブル・商品リコール時に10倍に増加）
※想定値
...
|-----|----------|----------------|---------|
| 可用性 | 99.5% | 0.5% | 30日間ローリングウィンドウ |
| APIレイテンシ | 95%ile: 1秒以内 | 5% | 30日間ローリングウィンドウ |
| 画面レイテンシ | 95%ile: 3秒以内 | 5% | 30日間ローリングウィンドウ |
...
| 対象 | 95%ile | 測定期間 |
|------|-------|---------|
| 問い合わせ受付API | 1秒以内 | 30日間ローリングウィンドウ |
| 管理画面 | 3秒以内 | 30日間ローリングウィンドウ |
...
**週次**
- 30日間ローリングウィンドウのSLO達成状況
- エラーバジェット消費傾向
- ボトルネック分析

**月次**
- 30日間ローリングウィンドウのSLO達成状況（月末時点）
- SLO未達時の原因分析
- SLO目標値の妥当性評価
