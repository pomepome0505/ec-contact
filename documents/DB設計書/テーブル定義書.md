...
| inquiry_categories | 問い合わせカテゴリ | 問い合わせのカテゴリマスタ |
...
| is_active | 有効フラグ | BOOLEAN | NO | true | - | - | false の場合ログイン不可 |
| is_admin | 管理者フラグ | BOOLEAN | NO | false | - | - | true の場合、従業員管理・カテゴリ管理が可能 |
| temporary_password_expires_at | 一時パスワード有効期限 | TIMESTAMP | YES | NULL | - | - | 管理者によるパスワードリセット時に設定、パスワード変更後にNULLに戻る |
| created_at | 作成日時 | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | - | - |
| updated_at | 更新日時 | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | - | - |

---

## 2. inquiry_categories (問い合わせカテゴリ)

### 概要
問い合わせのカテゴリを管理するマスタテーブル。管理画面から追加・編集・有効/無効の切替が可能。
...
|--------|--------|----------|------|-------------|------------|---------|------|
| id | ID | BIGINT UNSIGNED | NO | AUTO_INCREMENT | PRIMARY KEY | - | 主キー |
| name | カテゴリ名 | VARCHAR(50) | NO | - | - | - | 画面表示用のカテゴリ名 |
| display_order | 表示順 | INTEGER | NO | 0 | - | - | 一覧画面やフォームでの並び順 |
| is_active | 有効フラグ | BOOLEAN | NO | true | - | - | false の場合、問い合わせフォームの選択肢に表示されない |
| created_at | 作成日時 | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | - | - |
| updated_at | 更新日時 | TIMESTAMP | NO | CURRENT_TIMESTAMP | - | - | - |
...
| staff_id | 担当者ID | BIGINT UNSIGNED | YES | NULL | INDEX | users.id (ON DELETE RESTRICT) | 割り当てられたサポート担当者のID |
| order_number | 注文番号 | VARCHAR(50) | YES | NULL | - | - | 顧客の注文番号 (任意) |
| category_id | カテゴリID | BIGINT UNSIGNED | NO | - | INDEX | inquiry_categories.id (ON DELETE RESTRICT) | 問い合わせカテゴリ |
| channel | 受付区分 | VARCHAR(20) | NO | 'form' | - | - | 'form'（フォーム）, 'phone'（電話） |
| customer_name | 顧客氏名 | VARCHAR(100) | NO | - | - | - | 問い合わせした顧客の氏名 |
| customer_email | 顧客メールアドレス | VARCHAR(100) | YES | NULL | - | - | 返信メールの送信先。電話問い合わせでは未取得の場合あり |
| status | ステータス | VARCHAR(20) | NO | 'pending' | INDEX | - | 'pending', 'in_progress', 'resolved', 'closed' |
| priority | 優先度 | VARCHAR(20) | NO | 'medium' | INDEX | - | 'low', 'medium', 'high', 'urgent' |
...
| 値 | 日本語表記 | 説明 |
|----|-----------|------|
| form | フォーム | 顧客が問い合わせフォームから送信 |
| phone | 電話 | 担当者が電話での問い合わせをシステムに登録 |
...
| staff_id | 担当者ID | BIGINT UNSIGNED | YES | NULL | INDEX | users.id (ON DELETE RESTRICT) | 送信者が担当者の場合のID（message_type='staff_reply'の場合のみ） |
| message_type | メッセージ種別 | VARCHAR(20) | NO | - | - | - | 'initial_inquiry', 'customer_reply', 'staff_reply' |
